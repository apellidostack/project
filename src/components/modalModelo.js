...
function AddModelo({ isOpenModelo, onCloseModelo, childrenModelo }){
  ...
  const handleSubmit = async(event) => {
    ...
    try{
      ...
      const logear = await fetch('http://localhost:4000/modelo/set',{
        method: 'POST',
      ...
      setFormData({...formData, ['descripMod']: ''});
    ...
    return(
        ...
        <div className='fila'>
          <label htmlFor="user">MODELO:</label>
          <input type="text" id="descripMod" name="descripMod" value={formData.descripMod} onChange={handleChange}/>
        </div>
        <button type="submit" onClick={handleSubmit}>REGISTRAR</button>
        </form>
        
        <button className="modal-close" onClick={onCloseModelo}>
          &#x2715;
        </button>
        {childrenModelo}
      </div>
