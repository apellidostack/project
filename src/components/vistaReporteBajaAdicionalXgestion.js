...
import PDFGeneratorAdicional from './reporteEquiposAdicionales';
import PersistentDrawerLeft from './navMenu';
import PDFGenerarBajaEquiposAdicionales from './PDFBajasEquiposAdicionales';

function ReporteBajaDeEquipoAdicional() {
 ...
 const [labs, setLabs] = useState([]); // Variable para almacenar las opciones
  ...
  const selectLabs = async ()=>{
    try{
      ...
      const datos = await fetch('http://localhost:4000/fechas/baja/equipo/adicional/por/gestion',{
...
useEffect(()=>{
  ...
  const handleChangeSelect = async (e)=>{
    try{
      // Aquí puedes realizar alguna acción con los datos del formulario, como enviarlos a un servidor
      const datos = await fetch('http://localhost:4000/reporte/baja/equipo/adicional/gestion',{
        method: 'POST',
 ...
 */
  ...
  return (
    ...
    <section className="container">
        <h3>Generar PDF de Baja de Equipos Adicionales por Gestión</h3>
    <div className='col'>
      ...
      <div className="ReportePdf">
        ...
        {(opciones.length > 0)?(
            <PDFGenerarBajaEquiposAdicionales info={opciones} nombre={nomRep}/>
        ):null}
...
export default ReporteBajaDeEquipoAdicional;
