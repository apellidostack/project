...
import PDFMovimientoEquipoAdicional from './reporteMovimientoEquipoAdicional';

function ReporteMovimientoEquipoAdicional() {
 ...
 const [labs, setLabs] = useState([]); // Variable para almacenar las opciones
  ...
  const selectLabs = async ()=>{
    try{
      ...
      const datos = await fetch('http://localhost:4000/get/fechas/movimiento/equipo/adicional',{
...
useEffect(()=>{
  ...
  const handleChangeSelect = async (e)=>{
    try{
      // Aquí puedes realizar alguna acción con los datos del formulario, como enviarlos a un servidor
      const datos = await fetch('http://localhost:4000/get/reporte/movimiento/equipo/adicional/por/gestion',{
        method: 'POST',
 ...
 */
  ...
  return (
    ...
    <section className="container">
        <h3>Generar PDF de Movimiento de Equipos Adicionales por Gestión</h3>
    <div className='col'>
      ...
      <div className="ReportePdf">
        ...
        {(opciones.length > 0)?(
            <PDFMovimientoEquipoAdicional info={opciones} nombre={nomRep}/>
        ):null}
...
export default ReporteMovimientoEquipoAdicional;
