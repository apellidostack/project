...
//
const PDFMovimientoDeEquiposXGestion = (props) => {
    //console.log(props.info.resultPosterior);
    return (
        <PDFViewer style={{ width: '100%', height: '100vh' }}>
      <Document>
      {Object.entries(props.info.resultPosterior).map(([codMov, result]) => (
      <PDFIterar
        key={codMov} 
        info={result}
        nombre={props.nombre}
        detalleMov={props.info.detalleMov[codMov]}
        labMov={props.info.labMov[codMov]}
      />
    ))}
          </Document>
    </PDFViewer>
...
const PDFIterar = (props) => {
  const elementosPosterior = {};
  //fecha
  console.log('hola reporte');
  console.log(props.info);
  //throw('xd')
  const fechEjec=new Date(props.info[0][0].FechaEjec);
  ...
  return (
    <>
        ...
        <View style={styles.container}>
          ...
          {/* <View style={styles.datos}>
            <Text style={[styles.bordeComp,{backgroundColor: 'blue',color: 'white',padding: '5px'}]}>AUTORIZADO POR: {props.info[0][0].Autorizador}</Text>
            <Text style={[styles.bordeComp,{backgroundColor: 'blue',color: 'white',padding: '5px'}]}>REALIZADO POR: {props.info[0][0].Ejecutor}</Text>
          </View> */}
          <View style={[styles.centrar,{marginBottom: '10px'}]}>
        <View style={styles.datos2}>
            <Text style={[styles.bordeComp2,{backgroundColor: 'blue',color: 'white',width: '18%'}]}>Se movio</Text>
            <Text style={[styles.bordeComp2,{backgroundColor: 'blue',color: 'white',width: '18%'}]}>como</Text>
        </View>
        {props.detalleMov.map((dM)=>{
              return(
                <View style={styles.datos}>
                <Text style={[styles.bordeComp2,{backgroundColor: 'rgb(255, 247, 203)',width: '18%'}]}>{dM.nomAnterior}</Text>
                <Text style={[styles.bordeComp2,{backgroundColor: 'rgb(255, 247, 203)',width: '18%'}]}>{dM.nomNuevo}</Text>
                </View>
              )
            })}
</View>
...
export default PDFMovimientoDeEquiposXGestion;
