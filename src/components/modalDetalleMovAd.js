...
function AutorizarMovEqAd({ isOpenAutorizarMovEqAd, onCloseAutorizarMovEqAd, childrenAutorizarMovEqAd, infoMovEq}){
...
const storedUserData = JSON.parse(storedJsonString);
  ...
  const handleSubmit = async(event) => {
    ...
    try{
      ...
      const logear = await fetch('http://localhost:4000/autorizar/movimiento/adicional',{
        method: 'PUT',
    ...
    return(
        <div className="modal-overlay">
            <div className="modal">
              ...
              <div>
                <h2>Solicitud de movimiento de equipo adicional</h2>
                ...
                <p>Fecha de solicitud: {infoMovEq.detalleMov.labMov[0].FechaS}</p>
                <p>{`Se solicita movimiento de equipos adicionales de laboratorio: ${infoMovEq.detalleMov.labMov[0].labAnterior+' al laboratorio: '+infoMovEq.detalleMov.labMov[0].NroLab} `}</p>
                
                </div>
                ):null}
                <p>Equipos incluidos en la solicitud:</p>
                {infoMovEq.detalleMov.detalleMov.map((valueDet)=>(  
                  <div>{valueDet.nomAdAnterior}  se movera como  {valueDet.nomAdNuevo}</div>
                )
                )}
              </div>
              <div><button className='enviar2' value={`${infoMovEq.detalleMov.detalleMov[0].CodMovAd}/autorizada`} onClick={handleSubmit}>Autorizar</button><button className='enviar2' value={`${infoMovEq.detalleMov.detalleMov[0].CodMovAd}/rechazada`} onClick={handleSubmit}>Rechazar</button></div>
...
export default AutorizarMovEqAd