  ...
  return (
    <View style={styles.container}>
      <View style={{fontSize: '10px', textAlign: 'center', width: '170px'}}>
        ...
        <Text style={{border: '1px solid black', marginBottom: '10px', textAlign: 'center',marginTop: '30px', width:'80%', padding: '3px', fontSize: '16px', fontFamily: 'Times-Roman'}}>ALTA DE EQUIPOS DE COMPUTACIÓN EN {format(new Date(props.infoComp[0].FechaEjec), 'dd/MM/yyyy')}</Text>
        </View>
...
const PDFAltaEquipoAnual = (props) => {
...
props.info.forEach((elemento) => {
  const nombreEq = elemento.IdAlta;
  const fechEjec = new Date(elemento.FechaAlta);
...
export default PDFAltaEquipoAnual;
