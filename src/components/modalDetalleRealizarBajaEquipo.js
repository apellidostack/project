...
function EquipoParaDarBaja({ isOpenAutorizarMovEq, onCloseAutorizarMovEq, childrenAutorizarMovEq, infoMovEq}){
...
const storedUserData = JSON.parse(storedJsonString);
  ...
  const handleSubmit = async(event) => {
    ...
    try{
      ...
      const logear = await fetch('http://localhost:4000/realizar/baja',{
        method: 'POST',
        ...
        <div className="modal-overlay">
            <div className="modal">
              ...
              <div>
                ...
                <p>{`Baja de equipos de laboratorio: ${infoMovEq.detalleMov.detalleMov[0].NroLab}`}</p>
                
                </div>
                ):null}
                <p>Equipos a dar de Baja:</p>
                {infoMovEq.detalleMov.detalleMov.map((valueDet)=>(
                  <div>{valueDet.NombreEq}</div>
                )
                )}
              </div>
              <div><button className='enviar2' onClick={handleSubmit}>Realizar</button></div>
...
export default EquipoParaDarBaja