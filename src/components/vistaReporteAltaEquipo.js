...
import PDFAltaEquipoAnual from './PDFAltaEquipoAnual';

function ReporteAltaDeEquipo() {
 ...
 const [labs, setLabs] = useState([]); // Variable para almacenar las opciones
  ...
  const selectLabs = async ()=>{
    try{
      ...
      const datos = await fetch('http://localhost:4000/get/fechas/alta/equipo',{
...
useEffect(()=>{
  ...
  const handleChangeSelect = async (e)=>{
    try{
      // Aquí puedes realizar alguna acción con los datos del formulario, como enviarlos a un servidor
      const datos = await fetch('http://localhost:4000/get/reporte/alta/equipos',{
        method: 'POST',
 ...
 */
  ...
  return (
    ...
    <section className="container">
        <h3>Generar PDF de Alta de Equipos por Gestión</h3>
    <div className='col'>
      ...
      <div className="ReportePdf">
        ...
        {(opciones.length > 0)?(
            <PDFAltaEquipoAnual info={opciones} nombre={nomRep}/>
        ):null}
...
export default ReporteAltaDeEquipo;
