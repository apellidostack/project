...
import EquiposAdicionalesParaMover from './modalDetalleRealizarMovAd';
...
function SolicitarMovimientoAdicional(){
...
//movimientos autorizados
 ...
 const listaMovimientoAutorizados = async ()=>{
   try{
     // Aquí puedes realizar alguna acción con los datos del formulario, como enviarlos a un servidor
     const datos = await fetch('http://localhost:4000/lista/movimientos/adicionales/autorizados',{
     })
...
//
  ...
  useEffect(()=>{
    ...
    const handleChangeSelect=async(e)=>{
      ...
      try{
        // Aquí puedes realizar alguna acción con los datos del formulario, como enviarlos a un servidor
        const logear = await fetch('http://localhost:4000/get/equipoAdicional/para/mover',{
          method: 'POST',
  ...
  const handleSubmit = async(event) => {
    ...
    try{
      ...
      setMensaje('Solicitud Realizada');
      //console.log(respuesta);
    ...
    <section className="container">
      <div className='solicitudes'>
          <div className='row-grid'>
            <div><h1>Solicitar Movimiento de Equipo Adicional</h1></div>
            <div className='col'>
            ...
            <div className='lb-imp'>
            <div className='lb-center'>
              <label>Mover a</label></div>
              <select name='labDestino' onChange={handleChangeLab}>
                <option value="0">Seleccionar</option>
                {(labs.length > 0)?(labs.map((lb)=>(
                  <option key={lb.NroLab} value={lb.NroLab}>{lb.GrupoTrabajo}</option>
                ))):null}
    ...
    <div>
            <EquiposAdicionalesParaMover isOpenAutorizarMovEq={modalOpenAMovEq} onCloseAutorizarMovEq={handleCloseModalAMovEq} infoMovEq={{detalleMov}}>

            </EquiposAdicionalesParaMover>
            </div>
      <div>
...
export default SolicitarMovimientoAdicional