...
function UpdateValorItem({ isOpenUV, onCloseUV, datosUpv, childrenUV }){
  ...
  const handleSubmit = async(event) => {
    ...
    try{
      ...
      const logear = await fetch('http://localhost:4000/update/valor',{
        method: 'PUT',
        body: JSON.stringify({formData,datosUpv}),
    ...
    return(
        ...
        <form>
            <div>
          <h1>FORM VALOR ITEM</h1>
        </div>
        <div className='fila'>
          <label htmlFor="user">VALOR:{datosUpv}</label>
        ...
        <button className="modal-close" onClick={onCloseUV}>
          &#x2715;
        </button>
        {childrenUV}
      </div>
...
export default UpdateValorItem