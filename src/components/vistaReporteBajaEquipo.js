...
import PDFBajaEquipoAnual from './PDFBajaEquipoAnual';

function ReporteBajaDeEquipo() {
 ...
 const [labs, setLabs] = useState([]); // Variable para almacenar las opciones
  ...
  const selectLabs = async ()=>{
    try{
      ...
      const datos = await fetch('http://localhost:4000/get/fechas/baja/equipo',{
...
useEffect(()=>{
  ...
  const handleChangeSelect = async (e)=>{
    try{
      // Aquí puedes realizar alguna acción con los datos del formulario, como enviarlos a un servidor
      const datos = await fetch('http://localhost:4000/get/reporte/baja/equipos',{
        method: 'POST',
 ...
 */
  ...
  return (
    ...
    <section className="container">
        <h3>Generar PDF de Baja de Equipos por Gestión</h3>
    <div className='col'>
      ...
      <div className="ReportePdf">
        ...
        {(opciones.length > 0)?(
            <PDFBajaEquipoAnual info={opciones} nombre={nomRep}/>
        ):null}
...
export default ReporteBajaDeEquipo;
