  ...
  return (
    <div className="ReportePdf">
      <div className='ContenedorEquipos'>
        {opciones.map((datos) => (
            <div className='col'>
            <button type="button" name={datos.NombreEq} id={datos.NombreEq} key={datos.NombreEq} value={datos.NombreEq} onClick={handleSubmit}>
                {datos.NombreEq}
            </button>
            </div>
        ))}
        ...
        {(detalles.length > 0)?(
            <PDFGenerator info={detalles} nombre={nomRep}/>
        ):null}
