...
function AutorizarMovEq({ isOpenAutorizarMovEq, onCloseAutorizarMovEq, childrenAutorizarMovEq, infoMovEq}){
...
const storedUserData = JSON.parse(storedJsonString);
  ...
  const handleSubmit = async(event) => {
    ...
    try{
      ...
      const logear = await fetch('http://localhost:4000/autorizar/movimiento/equipo',{
        method: 'PUT',
        ...
        <div className="modal-overlay">
            <div className="modal">
              ...
              <div>
                ...
                <p>Fecha de solicitud: {infoMovEq.detalleMov.personalMov[0].FechaSol}</p>
                <p>{`Se solicita movimiento de equipos de laboratorio: ${infoMovEq.detalleMov.labMov[0].labAnterior+' al laboratorio: '+infoMovEq.detalleMov.labMov[0].NroLab} `}</p>
                
                </div>
                ):null}
                <p>Equipos incluidos en la solicitud:</p>
                {infoMovEq.detalleMov.detalleMov.map((valueDet)=>(
                  <div>{valueDet.nomAnterior} mover como {valueDet.nomNuevo}</div>
                )
                )}
              </div>
              <div><button className='enviar2' value={`${infoMovEq.detalleMov.detalleMov[0].CodMov}/autorizada`} onClick={handleSubmit}>Autorizar</button><button className='enviar2' value={`${infoMovEq.detalleMov.detalleMov[0].CodMov}/rechazada`} onClick={handleSubmit}>Rechazar</button></div>
