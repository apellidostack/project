...
function EquiposAdicionalesParaDarBaja({ isOpenAutorizarMovEq, onCloseAutorizarMovEq, childrenAutorizarMovEq, infoMovEq}){
...
const storedUserData = JSON.parse(storedJsonString);
  ...
  const handleSubmit = async(event) => {
    ...
    try{
      ...
      const logear = await fetch('http://localhost:4000/realizar/baja/equipo/adicional',{
        method: 'POST',
        ...
        <div className="modal-overlay">
            <div className="modal">
              ...
              <div>
                ...
                <p>Dar de baja los siguientes equipos adicionales:</p>
                {infoMovEq.detalleMov.detalleMov.map((valueDet)=>(
                  <p>{valueDet.CodEquipoAd}</p>
                )
                )}
              </div>
...
export default EquiposAdicionalesParaDarBaja