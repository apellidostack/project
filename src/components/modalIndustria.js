...
function AddIndustria({ isOpenIndustria, onCloseIndustria, childrenIndustria }){
  ...
  //
    ...
    if (!isOpenIndustria) {return null;}
  ...
  const handleSubmit = async(event) => {
    ...
    try{
      ...
      const logear = await fetch('http://localhost:4000/industria/set',{
        method: 'POST',
      ...
      setFormData({...formData, ['descripIn']: ''});
    ...
    return(
        ...
        <form>
            <div>
          <h1>AGREGAR INDUSTRIA</h1>
        </div>
        <div className='fila'>
          <label htmlFor="user">INDUSTRIA:</label>
          <input type="text" id="descripIn" name="descripIn" value={formData.descripIn} onChange={handleChange}/>
        </div>
        <button className='enviar' type="submit" onClick={handleSubmit}>REGISTRAR</button>
        </form>
        
        <button className="modal-close" onClick={onCloseIndustria}>
          &#x2715;
        </button>
        {childrenIndustria}
      </div>
