...
import AutorizarMovEqAd from './modalDetalleMovAd';
import TablaAutorizarBajaAdicional from './vistaAutorizarBajaAdicional';


//
function TablaAutorizarMovAd(){
  //detalle
  const [detalleMov, setDetalleMov] = useState([]);
const detalleMovEq = async(codD) => {
  try{
    // Aquí puedes realizar alguna acción con los datos del formulario, como enviarlos a un servidor
    const logear = await fetch('http://localhost:4000/lista/detalle/movimiento/adicional',{
      method: 'POST',
...
//detalle movimiento equipo
      ...
      //datos de personal
      const [lSolicitudesMovEqAd, setLSolicitudesMovEqAd] = useState([]);

  const listaSolicitudes = async ()=>{
    try{
      // Aquí puedes realizar alguna acción con los datos del formulario, como enviarlos a un servidor
      const datos = await fetch('http://localhost:4000/lista/movimientos/adicionales/pendientes',{
      })
      ...
      setLSolicitudesMovEqAd(respuesta); // Actualiza las opciones con los datos de la respuesta
      //navigate('/motivo');
    }catch(error){
      //console.error('Error en la solicitud',error.message);
      alert('El Usuario no existe')
  ...
  useEffect(()=>{
    console.log(lSolicitudesMovEqAd);
  },[lSolicitudesMovEqAd])
    ...
    <div className='contaniner-nav'>
      ...
      <section className="container">
        ...
        <div><h3>Tabla Movimientos Adicionales Autorizadas</h3></div>
        {lSolicitudesMovEqAd.length === 0 ? (
        <div>No rows</div>
      ) : (
      <DataGrid
        rows={lSolicitudesMovEqAd}
        columns={columns}
        initialState={{
          pagination: {
            paginationModel: {
      ...
      />
        ...
        <TablaAutorizarBajaAdicional />

        <AddCargo isOpenCargo={modalOpenPer} onCloseCargo={handleCloseModalP}>
            
          </AddCargo>
          <div>
            <AutorizarMovEqAd isOpenAutorizarMovEqAd={modalOpenAMovEq} onCloseAutorizarMovEqAd={handleCloseModalAMovEq} infoMovEq={{detalleMov}}>
...
export default TablaAutorizarMovAd